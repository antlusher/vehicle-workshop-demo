...
router.get('/customers/:id/stats', async (req, res) => {
  ...
  const jobs = jobRows.map((r) => {
    ...
    return {
      id: r.id, openedAt: r.created_at, closed: r.closed, registration: r.registration,
      vehicle: [r.make, r.model, r.year].filter(Boolean).join(' '),
      reportTotal: r.report_total ? parseFloat(r.report_total) : null,
      publishedAt: r.published_at,
      quoteId: r.quote_id || null, quoteRef: r.quote_ref || null,
      quoteStatus: r.quote_status || null, quoteTotal, quoteDate: r.quote_date || null,
    };
  });
...
// Vehicle search for customer linking
